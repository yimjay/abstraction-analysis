...
int main(int argc, char **argv) 
{
  ...
  j = MS;
  T1_size=n; 
  T1_rep=CacheSZ / T1_size + 1; 
  T1_buf = calloc(T1_size*T1_rep, sizeof(double));
  T2_size=n; 
  T2_rep=CacheSZ / T2_size + 1; 
  T2_buf = calloc(T2_size*T2_rep, sizeof(double));
  S_size=j*n; 
  S_rep=CacheSZ / S_size + 1; 
  S_buf = calloc(S_size*S_rep, sizeof(double));
  V_size=j*n; 
  V_rep=CacheSZ / V_size + 1; 
  V_buf = calloc(V_size*V_rep, sizeof(double));
  #define DO_FLUSH 1
  ...
  for (__pt_MT_ivar=0; __pt_MT_ivar<MT; ++__pt_MT_ivar) {
    srand(RANDSEED);
    for (__pt_i0=0; __pt_i0<T1_size *T1_rep; ++__pt_i0)
    {
      T1_buf[__pt_i0] = rand();; 
    }
    T1 = T1_buf;
    for (__pt_i0=0; __pt_i0<T2_size *T2_rep; ++__pt_i0)
    {
      T2_buf[__pt_i0] = rand();; 
    }
    T2 = T2_buf;
    for (__pt_i0=0; __pt_i0<S_size *S_rep; ++__pt_i0)
    {
      S_buf[__pt_i0] = rand();; 
    }
    S = S_buf;
    for (__pt_i0=0; __pt_i0<V_size *V_rep; ++__pt_i0)
    {
      V_buf[__pt_i0] = rand();; 
    }
    V = V_buf;
